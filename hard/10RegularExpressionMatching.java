            ...
            for (int j = 0; j < p.length(); j++) {
                ...
                if (p.charAt(j) == '*') {
                    if (p.charAt(j-1) != s.charAt(i) && p.charAt(j-1) != '.') {
						...
						//1 if appear 0 time meaning no this element, check dp[i+1][j-1]
						//2 if appear 1 time meaning first apperance, check dp[i+1][j]
						//3 if appear multi-time, check dp[i][j+1]
